...
const ServicesPage: React.FC = () => {
    ...
    const handleAddService = async () => {
        ...
        try {
            ...
            fetchServices(); // Refetch to get the new list
                ...
                <div className="bg-smartfix-darker rounded-2xl shadow-xl border border-smartfix-dark overflow-hidden">
                    <div className="grid grid-cols-1 divide-y divide-smartfix-dark">
                        {services.map((service) => (
                            <div key={service.id} className="p-6 flex justify-between items-start hover:bg-smartfix-dark transition-colors duration-200">
                                <div>
                                ...
                                <div className="text-right flex items-center shrink-0 ml-4">
                                     <p className="text-xl font-semibold text-smartfix-lightest mr-4">{service.price.toLocaleString('ru-RU')} ₽</p>
                                     {user?.role === Role.Admin && (
                                        <button onClick={() => handleDeleteService(service.id)} className="text-red-500 hover:text-red-400 p-2 rounded-full hover:bg-red-900/50 transition-colors">
